#!/usr/bin/env python3
"""
cvpr_format_occ_gen_v3 — Ultimate Pipeline
Features:
1. Ego Pose Backend: kiss_slam (0.5820 mIoU with 40 sweeps)
2. Sweeps: Default 40
...
def generate_occupancy(nusc, scene_name, pose_dict, num_sweeps=40, box_fill=True):
    ...
    while sample_token:
        ...
        for _ in range(num_sweeps):
            ...
            p, s = keyframe_align_with_pose(prev_i, curr_info, pose_dict[prev_i['utime']] if pose_dict else prev_i['gt_pose'], curr_pose)
        ...
        for _ in range(num_sweeps):
            ...
            p, s = keyframe_align_with_pose(next_i, curr_info, pose_dict[next_i['utime']] if pose_dict else next_i['gt_pose'], curr_pose)
            pcs.append(p); segs.append(s)
    ...
    pbar.close()
    if box_fill:
        print(f"Total boxed filled: {total_filled}")
...
def main():
    ...
    parser.add_argument('--no_box_fill', action='store_true',
                        help='Disable 3D box interior filling')
    ...
    occ_list, token_list = generate_occupancy(nusc, args.scene, pose_dict, args.sweeps,
                                              box_fill=not args.no_box_fill)
