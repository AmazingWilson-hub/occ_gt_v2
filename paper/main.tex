...
\begin{abstract}
This paper presents an extension to a prior auto-labeling pipeline for projected semantic labeling and Bird's-Eye-View (BEV) map generation. The proposed method replaces the original front-end semantic labeling module with a stronger multi-view segmentation and projection framework that combines YOLOv7, SAM~3, and a GPT-4o-guided prompt-refinement loop to improve image-space semantics for both dynamic agents and static scene elements. The refined semantics are projected into LiDAR space through explicit camera--LiDAR calibration, fused across views and frames, and rasterized into semantic BEV maps. Segment Anything in LiDAR (SAL) is adopted as the primary baseline because it follows a comparable 2D-to-3D auto-labeling paradigm. Under the shared super-class projected-label protocol used in this study, the proposed method improves mIoU on the nuScenes validation split from 52.6\% to 58.7\%. Qualitative results on Taiwan traffic scenes further show cleaner and more coherent BEV visualizations. These findings indicate that improving the 2D semantic front-end, together with calibration-based projection and multi-view fusion, can substantially enhance downstream 3D pseudo-label quality in projection-based BEV pipelines. The same accumulated semantic point cloud is also rasterized into a $200\times200\times16$ 3D occupancy grid; with ground-truth ego poses, the pipeline achieves 72.27\% mIoU on nuScenes, and with KISS-SLAM estimated poses it achieves 58.20\%, demonstrating practical occupancy label generation without ground-truth localization.
\end{abstract}
...
This issue becomes more severe under domain shift. Public datasets such as nuScenes~\cite{nuscenes}, Zenseact Open Dataset~\cite{tbd_zod}, and SemanticKITTI~\cite{tbd_semantickitti} are important benchmarks, but they do not fully capture the traffic patterns of Taiwan, where dense scooters, narrow alleys, and heavy roadside signage frequently produce small, distant, and partially occluded instances. Furthermore, real-world deployment datasets often lack ground-truth ego poses, making multi-frame point cloud accumulation---a key step for dense occupancy label generation---dependent on estimated ego localization. Figure~\ref{fig:taiwan-vs-public} illustrates the type of mixed-traffic scene that motivates this work.
...
\begin{itemize}
  ...
  \item We extend the pipeline to generate dense 3D semantic occupancy labels via adaptive multi-frame LiDAR accumulation, dynamic object volume filling, and a pose estimation module that supports datasets without ground-truth localization, achieving 72.27\% mIoU with GT poses and 58.20\% with KISS-SLAM on nuScenes.
\end{itemize}
...
Figure~\ref{fig:system-overview} summarizes the extended pipeline. The framework contains three stages: (1) image-space semantic segmentation and refinement, (2) camera-to-LiDAR semantic projection and fusion, and (3) downstream semantic BEV map generation and 3D occupancy label generation from the resulting semantic point cloud.
...
Figure~\ref{fig:occ-bev} shows a representative occupancy BEV visualization on nuScenes, where each cell is colored by its majority semantic class. Figure~\ref{fig:occ-sweep} compares single-frame versus 40-frame accumulation, illustrating how longer stacking substantially densifies the occupancy grid and reduces empty voxels in road and structure regions. Figure~\ref{fig:occ-pose} shows the effect of pose estimation quality: GT poses produce sharp, well-aligned voxel boundaries, while KISS-SLAM poses introduce mild misalignment in regions far from the ego vehicle.

\begin{figure}[t]
  \centering
  \includegraphics[width=\linewidth]{occ_bev_visualization.png}
  \caption{Representative semantic occupancy BEV on nuScenes. Each cell is colored by its majority semantic class using the 17-class label set. Empty (free) voxels are shown in black.}
  \label{fig:occ-bev}
\end{figure}

\begin{figure}[t]
  \centering
  \subfloat[1 sweep]{\includegraphics[width=0.49\linewidth]{occ_sweep_1.png}\label{fig:occ-sweep-1}}
  \hfill
  \subfloat[40 sweeps]{\includegraphics[width=0.49\linewidth]{occ_sweep_40.png}\label{fig:occ-sweep-40}}
  \caption{Effect of multi-frame accumulation on occupancy density. A single sweep leaves large empty regions, while stacking 40 sweeps substantially fills road and structure areas.}
  \label{fig:occ-sweep}
\end{figure}

\begin{figure}[t]
  \centering
  \subfloat[GT pose]{\includegraphics[width=0.49\linewidth]{occ_pose_gt.png}\label{fig:occ-pose-gt}}
  \hfill
  \subfloat[KISS-SLAM pose]{\includegraphics[width=0.49\linewidth]{occ_pose_kiss.png}\label{fig:occ-pose-kiss}}
  \caption{Effect of ego pose quality on occupancy label accuracy. GT poses produce sharp voxel boundaries, while KISS-SLAM estimated poses introduce mild misalignment in regions far from the ego vehicle.}
  \label{fig:occ-pose}
\end{figure}
...
\begin{figure*}[t]
  \centering
  \subfloat[raw BEV points]{\includegraphics[width=0.29\textwidth]{BEV點雲原始圖_v1_cropped.png}\label{fig:bev-raw}}
  \hfill
  \subfloat[aggregated point-only BEV]{\includegraphics[width=0.29\textwidth]{bev_without3D.jpg}\label{fig:bev-without3d}}
  \hfill
  \subfloat[hybrid BEV with tracker boxes]{\includegraphics[width=0.29\textwidth]{bev_with3D.jpg}\label{fig:bev-with3d}}
...
\section{Conclusion}
We presented an extension of a prior auto-labeling pipeline for projected semantic labeling and BEV map generation. The key change is a stronger semantic front-end that improves image-space masks before camera-to-LiDAR projection. Under the shared super-class projected-label protocol used in this paper, the improved front-end and projection pipeline outperform SAL on nuScenes, the resulting 3D occupancy labels reach 72.27\% mIoU with GT poses and 58.20\% with KISS-SLAM, and the Taiwan examples qualitatively show cleaner BEV visualizations. These results support the value of improving image-space semantics before projection while leaving broader occupancy-level evaluation under more challenging localization settings to future work.
